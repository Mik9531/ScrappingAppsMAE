...
top10Paid_apps = pd.read_sql(
    "SELECT  TP.position, A.title, A.url, A.icon, A.created, A.summary, A.score FROM `TOP_PAID` TP INNER JOIN APPS A ON A.appId = TP.appId WHERE country = 'ALA' AND TP.CREATED = %s GROUP BY TP.appId ORDER BY POSITION LIMIT 10",
    params=[last_date_day],
    con=sqlEngine)
